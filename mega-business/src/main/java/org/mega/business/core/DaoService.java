...
public class DaoService {
	@PersistenceContext(unitName = "mega")
	EntityManager em;
	 ...
	 * @param arrEntities
	 *            is array Entities object
	 * @return number of Entities is deleted to database successfull
	 */
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	...
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	public Date getDate() {
		return Calendar.getInstance(zone, locale).getTime();
	}

	/**
	 * Get system date of database.
	 * 
	 * @return Date
	 */
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	...
	private Query getQuery(ParamQuery paramQuery, EntityManager em) {
		...
		if (query != null) {
			...
			if (paramQuery.getStartPosition() > 0) {
				query.setFirstResult(paramQuery.getStartPosition());
			}
