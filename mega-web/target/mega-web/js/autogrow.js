...
(function($) {
	...
	$.fn.autogrow = function(opts) {
		...
		, selector = that.selector, defaults = {
			...
			,
			fixMinHeight : true // if you don't want the box to shrink below its
								// initial size
			,
			...
			,
			onInitialize : false
		// resizes the textareas when the plugin is initialized
		};
		...
		that.each(function(i, elem) {
			...
			if (elem.is(':visible') || parseInt(elem.css('height'), 10) > 0) {
				...
				clone = elem.clone().addClass(opts.cloneClass).val(elem.val())
						.css({
							position : 'absolute',
							visibility : 'hidden',
							display : 'block'
		...
		function resize(e) {
			var box = $(this), oldHeight = box.innerHeight(), newHeight = this.scrollHeight, minHeight = box
					.data('autogrow-start-height') || 0, clone;
			if (oldHeight < newHeight) { // user is typing
				this.scrollTop = 0; // try to reduce the top of the content
									// hiding for a second
				opts.animate ? box.stop().animate({
					height : newHeight
				}, opts.speed) : box.innerHeight(newHeight);
			} else if (!e || e.which == 8 || e.which == 46
					|| (e.ctrlKey && e.which == 88)) { // user is deleting,
														// backspacing, or
														// cutting
				if (oldHeight > minHeight) { // shrink!
					...
					// make "invisible", remove height restriction potentially
					// imposed by existing CSS
					.css({
						position : 'absolute',
						zIndex : -10,
						height : ''
					...
					.val(box.val());
					box.after(clone); // append as close to the box as
										// possible for best CSS matching for
										// clone
					...
					newHeight++; // adding one back eliminates a wiggle on
									// deletion
					clone.remove();
					box.focus(); // Fix issue with Chrome losing focus from
									// the textarea.
